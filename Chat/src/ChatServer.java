...
public class ChatServer {
    ...
    private void start() {
        ...
        try (ServerSocketChannel server = ServerSocketChannel.open(); Selector selector = Selector.open()) {
            ...
            //绑定监听端口,这里不是给ServerSocketChannel绑定，而是给ServerSocket绑定，socket()就是获取通道原生的ServerSocket或Socket，socket是阻塞连接
            //server.socket().bind(new InetSocketAddress(port));
            //建议直接用server.bind，因为channel可以是非阻塞的
    ...
    private void handles(SelectionKey key, Selector selector) throws IOException {
        ...
        if (key.isReadable()) {
            ...
            //判断用户是否退出
            if (msg.equals("quit")) { //用户正常退出
                //解除该事件的监听
                key.cancel();
                //更新Selector
                selector.wakeup();
                System.out.println("客户端[" + client.socket().getPort() + "]下线了！");
            } else if (msg.equals("")) { //用户异常退出
                //解除该事件的监听
                key.cancel();
                //更新Selector
                selector.wakeup();
                System.out.println("客户端[" + client.socket().getPort() + "]异常退出了！");
            }
        }
    }
    ...
    private String receive(SocketChannel client) throws IOException {
        ...
        //用filp()转换读写模式后，limit=position，position=0。也就是说从0开始读，只能读到第position个。
        read_buffer.flip();
    ...
    private void sendMessage(SocketChannel client, String msg, Selector selector) throws IOException {
        ...
        for (SelectionKey key : selector.keys()) {
            ...
            if (!(key.channel() instanceof ServerSocketChannel) && !client.equals(key.channel()) && key.isValid()) {
                ...
                //用filp()转换读写模式后，limit=position，position=0。也就是说从0开始读，只能写到第position个。
                write_buffer.flip();
